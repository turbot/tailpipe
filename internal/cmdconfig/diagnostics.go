...
func DisplayConfig() {
	...
	if !slices.Contains(configFormats, diagnostics) {
		error_helpers.ShowWarning("invalid value for TAILPIPE_CONFIG_DUMP, expected values: config,config_json")
		return
	}
	...
	case "config":
		...
		fmt.Println(b.String()) //nolint:forbidigo // expected output
	case "config_json":
		...
		fmt.Println(string(jsonBytes)) //nolint:forbidigo // expected output
	}
