...
// this is the worker function run by all workers, which all read from the ParquetConverter channel
...
func (w *parquetConversionWorker) convertFile(jsonlFilePath string) (int64, error) {
	...
	selectQueryFormat := w.getViewQuery(partition.UnqualifiedName, w.converter.schema)
